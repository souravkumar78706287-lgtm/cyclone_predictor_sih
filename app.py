...
.news-alert {
    background: linear-gradient(135deg, #1d0d07, #0b1016 70%);
    border: 1px solid #733016;
...
if run:
    ...
    expected_landfall = datetime.now() + timedelta(
        hours=hours_to_landfall
    )
    ...
    st.session_state["expected_landfall"] = (
        expected_landfall
    )
...
if st.session_state.get(
    ...
    expected_landfall = st.session_state[
        "expected_landfall"
    ]
    ...
    render_html(
        ...
        <div class="news-alert">
            ...
            <div class="news-kicker">
                LIVE FORECAST BULLETIN · AI WEATHER DESK
            </div>

            <div class="news-headline">
                Cyclone expected to reach the coast in approximately
                {hours_to_landfall} hours
            </div>

            <div class="news-summary">
                The system is currently classified as
                <strong>{severity}</strong> with predicted winds of
                <strong>{predicted_wind:.1f} kt</strong>. The model estimates
                a potential landfall near {hit_lat:.2f}° latitude and
                {hit_lon:.2f}° longitude.
            </div>

            <div class="news-facts">

                <div>
                    <div class="news-fact-label">Expected landfall</div>
                    <div class="news-fact-value">
                        {expected_landfall.strftime("%d %b %Y, %I:%M %p")}
                    </div>
                </div>

                <div>
                    <div class="news-fact-label">Forecast wind</div>
                    <div class="news-fact-value">
                        {predicted_wind:.1f} kt · Risk {severity_score}/6
                    </div>
                </div>

                <div>
                    <div class="news-fact-label">Potential impact</div>
                    <div class="news-fact-value">
